...
struct User {
  int           order = 3;
  string        filename = "periodic_100.msh";
  string        model = "periodic";
  int           n_step = 100, output_interval = 2;
  Real          cfl = 1.0, tEnd = 1;

  static constexpr auto InitFunc = [](int dim, const Real* coord, int Nf, Real* u) {
    // if (coord[0] > 0.5) u[0] = 1.5;
    // else u[0] = 1;
    u[0] = 1 + 0.3 * Sin(2 * coord[0] * PI);
    u[1] = 0.5;
    u[2] = 0.0;
    u[3] = 1.0;
  };
...
///
BndConds::BndConds() : lower{0.0, 0.0}, upper{1.0, 0.05} {}
